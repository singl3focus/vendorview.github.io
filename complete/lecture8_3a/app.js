...
class App{
	constructor(){
		...
		const ambient = new THREE.HemisphereLight(0x555555, 0x999999, 0.3);
		this.scene.add(ambient);
		
		this.sun = new THREE.DirectionalLight( 0xAAAAFF, 1);//2.5 );
		...
		loader.load(
			...
			// called when the resource is loaded
			 ( gltf ) => {
                ...
                const scale = 0.6;
                flashLightMesh.scale.set(scale, scale, scale);
                ...
                let material = new SpotLightVolumetricMaterial();
                ...
                flashLight.add(this.spotlight);

                geometry = new THREE.BoxGeometry( 0.08, 0.08, 0.3 );
                material = new THREE.MeshBasicMaterial();
                const collider = new THREE.Mesh( geometry, material );
                collider.name = 'Collider';
                flashLight.add( collider );

                flashLight.position.set(-0.430, 0.877, 3.013);
                flashLight.rotateY(Math.PI/2);
    ...
    pickupFlashLight( controller = this.controllers[0] ){
        ...
        this.flashLight.quaternion.identity();
        controller.children[0].visible = false;
        controller.add( this.flashLight );
        controller.userData.flashLight = true;
